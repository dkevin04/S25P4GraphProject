...
public class Controller {
	 ...
	 *
	 * @param capacity the capacity given at runtime.
	 */
	...
	/**
	 * Attempts to find an artistNode, if found then a duplicate message appears. If
	 * not, then we add the node in the graph and then adds it to the hash table.
	 * The same is done for song. If it is added, then a new edge is also added.
	 *
	 * @param artist the artist being added
	...
	/**
	 * Much like insert logic, removing from graph and hash table if found. Also, we
	 * take in a int value from command processor telling us if its a song or artist
